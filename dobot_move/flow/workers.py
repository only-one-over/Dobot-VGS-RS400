...
The original ``workers.py`` has been split into three focused modules:
...
- :mod:`dobot_move.flow.camera_test_worker` — GUI-only camera test
  workers (``CaptureThread``, ``CameraTestWorker``) with Qt deps.
...
- Headless runtime → ``flow_executor``
- GUI / Qt code    → ``qt_workers`` / ``camera_test_worker``

.. note::

    Importing this shim pulls in Qt (via ``qt_workers`` and
    ``camera_test_worker``). Headless code paths should import from
...
# GUI-only camera test symbols (pulls in Qt + QImage)
from .camera_test_worker import CameraTestWorker, CaptureThread

...
- 运动/抓取流程标签页：流程新建、重命名、复制、删除和切换，以及点位管理、模块参数编辑、有序流程步骤、暂停和继续。
...
5. 在相机测试视图中验证检测。
6. 确认标定/点位，编辑流程并在主控制页选择主流程。
7. 运行当前编辑流程或主流程。
8. 当系统状态需要时使用暂停/恢复或清除错误。
9. 断开连接或关闭应用，允许 Worker 和硬件资源干净地停止。

应用启动后自动启动 Modbus，并在后台并发连接机器人和主流程所需相机。连接按钮在任务执行期间禁用，连接失败、反馈重建或模型加载不得阻塞窗口。5 秒观察窗口结束后后台继续重连，不产生启动故障锁。

手动运行前只读检查设备状态；缺失设备时提示具体原因并触发后台重连，不修改 `40001`。自动使能在 Worker 中执行，避免冻结界面。

...
架构:
  ...
  VisionThread (本模块)
    ...
    → 更新 TargetCache (target_end + 按采集时刻插值 pose 预计算 target_base)
    → pose_buffer 不可用时仅写 target_end，退化兼容

  ServoThread (本模块)
    → 固定周期读取 FeedbackCache + TargetCache
    → 主路径直接用 target_base (检测时刻已预计算)；fallback 用最新 pose 转 target_end
    → 计算误差 → ServoP 下发
    → 唯一允许发送 ServoP 的线程
...
from ..robot.motion_safety import validate_servo_p_params
...
class VisionThread:
    ...
    def _loop(self):
        while self._running:
            ...
            # TODO: 理想应使用 FramePacket.capture_time（帧采集时刻），
            # 但 capture_frames() 返回原始 rs.frame 不携带时间戳，
            # 用 perf_counter 近似采集完成时刻（与 capture_numpy_packet 语义一致）
            ...
            # ── 4. 转换到末端坐标 + 按采集时刻插值位姿预计算 target_base ──
            t0 = time.monotonic()
...
class ServoThread:
    ...
    目标获取优先级:
    1. target_base (主路径，VisionThread 检测时刻已预计算)
    2. target_end + 最新 current_pose → convert_to_base_coords (fallback)
    """
